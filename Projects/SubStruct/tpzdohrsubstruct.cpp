...
void TPZDohrSubstruct::Write(TPZStream &buf, int withclassid)
{
	...
	this->fKCInvert.Write(buf);
	this->fKCi.Write(buf, 0);
	this->fInvertedStiffness.Write(buf);
	this->fInvertedInternalStiffness.Write(buf);
...
void TPZDohrSubstruct::Read(TPZStream &buf, void *context)
{
	...
	this->fInvertedStiffness.Read(buf);
	this->fInvertedInternalStiffness.Read(buf);
