...
void TPZDohrSubstruct::Contribute_Kc(TPZMatrix &Kc) {
  int i;
  int j;
  for (i=0;i<fCoarseIndex.NElements();i++) {
...
void TPZDohrSubstruct::Print(std::ostream &out)
{
...
void TPZDohrSubstruct::Write(TPZStream &buf, int withclassid)
{
	TPZSaveable::Write(buf, withclassid);
	this->fzi.Write(buf, 0);
	TPZSaveable::WriteObjects(buf, fWeights);
	this->fPhiC_Weighted_Condensed.Write(buf,0);
	this->fPhiC.Write(buf,0);
	this->fNullPivots.Write(buf, 0);
	this->fLocalWeightedResidual.Write(buf,0);
	this->fLocalLoad.Write(buf, 0);
	this->fKeC_star.Write(buf, 0);
	this->fKCInvert.Write(buf, withclassid);
	this->fKCi.Write(buf, 0);
	this->fInvertedStiffness.Write(buf,withclassid);
	this->fInvertedInternalStiffness.Write(buf,withclassid);
	TPZSaveable::WriteObjects(buf, this->fInternalEqs);
}
void TPZDohrSubstruct::Read(TPZStream &buf, void *context)
{
	TPZSaveable::Read(buf, context);
	this->fzi.Read(buf, 0);
	TPZSaveable::ReadObjects(buf, fWeights);
	this->fPhiC_Weighted_Condensed.Read(buf,0);
	this->fPhiC.Read(buf,0);
	this->fNullPivots.Read(buf, 0);
	this->fLocalWeightedResidual.Read(buf,0);
	this->fLocalLoad.Read(buf, 0);
	this->fKeC_star.Read(buf, 0);
	fKCInvert.Read(buf,context);
	this->fKCi.Read(buf, 0);
	this->fInvertedStiffness.Read(buf,context);
	this->fInvertedInternalStiffness.Read(buf,context);
	TPZSaveable::ReadObjects(buf, fInternalEqs);
}

//template class TPZRestoreClass<TPZDohrSubstruct, TPZDOHRSUBSTRUCT_ID>;
