...
public:
   ...
   * Simple constructor
   */
  OOPError()   { *f_local = '\0'; f_lstate = f_to_global = 1; }
  ...
  void SetErrorFile ( char *file  )   { strcpy( f_local, file ); }
  void GSetErrorFile( char *gfile )   { strcpy( f_global, gfile ); }
  
  char *GetErrorFile()    { return( f_local ); }
  char *GGetErrorFile()   { return( f_global ); }

  
  /**
   * Redirects the calls to the global file
   */
  void SetToGlobal( int v )     { f_to_global = v; }
   ...
   */
  void SetErrorState( int state )   { f_lstate = state; }
  /**
