	...
	for(i=fTaskList.begin();i!=fTaskList.end();i++){
		...
		if(CanExecute(i->fVersion, i->fState)){
			if (i->fProc == fProc){
				...
				TM->NotifyAccessGranted(objid, objData, i->fState, ver, fObjPtr);
			}else{
...
void OOPMetaData::ReleaseAccess(OOPObjectId &id, OOPMDataState st, OOPDataVersion & ver){
	...
	for(k = 0; k<l_size;k++){
		if(i->fTaskId == id){
			...
			if (i->fState == st){
				bstate=true;
				if(st==EWriteAccess) fTaskWrite=0;
				if(i->fVersion == ver) {
					bversion=true;
					fTaskList.erase(i);
				}
...
*/
int OOPMetaData::HasWriteAccess(long ProcId){
	if (!fTaskWrite) return 0;
