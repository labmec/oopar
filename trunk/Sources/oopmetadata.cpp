...
void OOPMetaData::VerifyAccessRequests(){
	...
	if(fTrans != ENoTransition) return;
/*
...
*/
	...
	if(fAccessList.HasVersionAccessRequests(fVersion) && fTrans == ENoTransition) {
		SuspendReadAccess();
...
void OOPMetaData::ReleaseAccess(const OOPObjectId &taskid, const OOPMDataDepend &depend){
	...
	if(depend.State() == EVersionAccess) {
		...
		// remove this processor from the list of suspended read access
		...
		cout << "OOPMetaData::ReleaseAccess Send messages "
			" granting read access to the other processors"<< endl;
#ifndef WIN32
...
#warning "must send cancelsuspendaccessconfirmation"
	...
	if(fTrans == ECancelReadTransition) {
		...
		if(fSuspendAccessProcessors.size() == fReadAccessProcessors.size()) {
			fTrans = ENoTransition;
		}
	}
}
...
*/
void OOPMetaData::SubmitAccessRequest(const OOPObjectId &taskId, const OOPMDataDepend &depend, int processor){
...
bool OOPMetaData::HasReadAccess(const int Procid) const {
  ...
  while(i != fReadAccessProcessors.end() && *i != Procid) i++;
  if(i== fReadAccessProcessors.end()) return false;
  return true;
#else
  list<int>::const_iterator i = fReadAccessProcessors.begin();
  i=find(fReadAccessProcessors.begin(), fReadAccessProcessors.end(), Procid);
...
#warning "Send messages to other processors"
	...
	// send suspend messages to all processors and revoke the access granted
